...
bool perform_switch_emergency = false; //true if start from backup
...
int message_size = (EEPROM.read(MESSAGE_SIZE_UPPER) << 8) | EEPROM.read(MESSAGE_SIZE_LOWER);
  ...
  matrix.beginText(10,1,0xFFFFFF);
   ...
   for(int i=0; i<43; i++)
   { 
     char message = EEPROM.read(RUNNING_CODE + i); //get the value after the size address
  ...
  if(CAN.available())
  {
    ...
    if(checksum_validated == false)
    {
     ...
     else if(checksum_validated_new == false)
      {
        ...
        {
          if(perform_switch_emergency != true)
          {
...
void setPartitions()
{
  ...
  if(partition_state == 1)
  {
    ...
    if(perform_switch_emergency == true)
    {
      RUNNING_CODE = BACKUP_START;
    } else {
      RUNNING_CODE = ACTIVE_START;
    }
