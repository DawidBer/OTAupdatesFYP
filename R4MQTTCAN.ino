...
#include <EEPROM.h>
#include "wifissidandlogininfo.h"
...
const char username[] = BROKERUSERNAME;
const char password[] = BROKERPASSWORD;
...
//void brokerConnection(const char host[], const int port, const char username[], const char password[]);
...
void setup()
{
  ...
  if(!CAN.begin(CanBitRate::BR_250k))
  {
    ...
    //brokerConnection(broker, port, username, password);
...
void sendCANmessageData()
{
 ...
 int end_index = 8;
  ...
  for(int message_counter=1; message_counter<=number_of_messages; message_counter++)
  {
    ...
    CanMsg msg(CanStandardId(2), 8, message);
